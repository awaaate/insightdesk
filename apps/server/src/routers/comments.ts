        ...
        try {
          ...
          const baseQuery = ctx.db
            .select({
              ...
              ci_sentiment_level_id: DB.schema.comment_insights.sentiment_level_id,
              ci_sentiment_confidence: DB.schema.comment_insights.sentiment_confidence,
              ci_emotional_drivers: DB.schema.comment_insights.emotional_drivers,
              ci_sentiment_reasoning: DB.schema.comment_insights.sentiment_reasoning,
              // Sentiment fields
              ...
              ci2_primary_intention: DB.schema.comment_intentions.primary_intention,
              ci2_secondary_intentions: DB.schema.comment_intentions.secondary_intentions,
            ...
            .innerJoin(DB.schema.comment_insights, eq(DB.schema.comments.id, DB.schema.comment_insights.comment_id))
            // INNER JOIN to force insights exist
            .innerJoin(DB.schema.insights, eq(DB.schema.comment_insights.insight_id, DB.schema.insights.id))
            // INNER JOIN to force sentiment exists
            .innerJoin(DB.schema.sentiment_levels, eq(DB.schema.comment_insights.sentiment_level_id, DB.schema.sentiment_levels.id))
            // INNER JOIN to force intention exists
            .innerJoin(DB.schema.comment_intentions, eq(DB.schema.comments.id, DB.schema.comment_intentions.comment_id))
            .innerJoin(DB.schema.intentions, eq(DB.schema.comment_intentions.intention_id, DB.schema.intentions.id));
          ...
          if (filter) {
            // Date filters
            if (filter.startDate) {
              conditions.push(gte(DB.schema.comments.created_at, new Date(filter.startDate)));
            }
            if (filter.endDate) {
              conditions.push(lte(DB.schema.comments.created_at, new Date(filter.endDate)));
            }
            ...
            if (filter.businessUnit) {
              conditions.push(eq(DB.schema.insights.business_unit, filter.businessUnit));
            }
            ...
            if (filter.intentionType) {
              conditions.push(eq(DB.schema.intentions.type, filter.intentionType as any));
            }
            
            // Sentiment level filter
            if (filter.sentimentLevels && filter.sentimentLevels.length > 0) {
              conditions.push(inArray(DB.schema.sentiment_levels.level, filter.sentimentLevels as any));
            }
            
            // Confidence filter
            if (filter.minConfidence) {
              conditions.push(gte(DB.schema.comment_insights.confidence, filter.minConfidence / 10));
            }
          }
          ...
          const countQuery = ctx.db
            .select({ count: sql<number>`count(distinct ${DB.schema.comments.id})` })
            .from(DB.schema.comments)
            .innerJoin(DB.schema.comment_insights, eq(DB.schema.comments.id, DB.schema.comment_insights.comment_id))
            .innerJoin(DB.schema.insights, eq(DB.schema.comment_insights.insight_id, DB.schema.insights.id))
            .innerJoin(DB.schema.sentiment_levels, eq(DB.schema.comment_insights.sentiment_level_id, DB.schema.sentiment_levels.id))
          ...
          rawResults.forEach(row => {
            ...
            const existingInsight = comment.insights.find((i: any) => i.id === row.insight_id);
            if (!existingInsight) {
          ...
          // Sort
          const sortColumn = sortBy === 'created_at' || sortBy === 'updated_at' ? sortBy : 'created_at';
      ...
      try {
        ...
        const insights = await ctx.db
          .select({
            ...
            sentiment_confidence: DB.schema.comment_insights.sentiment_confidence,
          ...
          .innerJoin(DB.schema.insights, eq(DB.schema.comment_insights.insight_id, DB.schema.insights.id))
          .leftJoin(DB.schema.sentiment_levels, eq(DB.schema.comment_insights.sentiment_level_id, DB.schema.sentiment_levels.id))
        ...
        const intention = await ctx.db
          .select({
            ...
            secondary_intentions: DB.schema.comment_intentions.secondary_intentions,
        ...
        throw new Error(
          error instanceof Error ? error.message : "Failed to get comment with relations"
        );
      }
    }),
  ...
  getByIdsWithRelations: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        throw new Error(
          error instanceof Error ? error.message : "Failed to get comments by IDs"
        );
      }
    }),
  ...
  statsBySentiment: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        if (input?.startDate) {
          conditions.push(gte(DB.schema.comments.created_at, new Date(input.startDate)));
        }
        if (input?.endDate) {
          conditions.push(lte(DB.schema.comments.created_at, new Date(input.endDate)));
        }
        ...
        const stats = await ctx.db
          ...
          .from(DB.schema.comment_insights)
          .innerJoin(DB.schema.sentiment_levels, eq(DB.schema.comment_insights.sentiment_level_id, DB.schema.sentiment_levels.id))
          .innerJoin(DB.schema.comments, eq(DB.schema.comment_insights.comment_id, DB.schema.comments.id))
          .where(whereClause)
          .groupBy(
        ...
        throw new Error(
          error instanceof Error ? error.message : "Failed to get sentiment stats"
        );
      }
    }),
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const stats = await ctx.db
          ...
          .from(DB.schema.comment_intentions)
          .innerJoin(DB.schema.intentions, eq(DB.schema.comment_intentions.intention_id, DB.schema.intentions.id))
          .innerJoin(DB.schema.comments, eq(DB.schema.comment_intentions.comment_id, DB.schema.comments.id))
          .where(whereClause)
          .groupBy(
        ...
        throw new Error(
          error instanceof Error ? error.message : "Failed to get intention stats"
        );
      }
    }),
});
